...
function buildPrompt(messageBody: string): string {
    ...
    const isGreeting = /^(hi+|hii+|hello+|hey+|hai+|yo+|sup+|gm+|good morning|good night)\b/.test(lowerMessage.trim());
...
Rules:
- Keep replies short: one line is preferred, two lines only if truly needed.
- Do not repeat the same word or phrase more than once.
...
- Do not sound formal, scripted, or robotic.
- Match the user's tone, but stay in English.
- If the user asks who you are or what your name is, reply: "I am Nandu."
- If the message is just a greeting, reply with a simple casual greeting like "Hi" or "Hey".
- Do not use filler lines, long paragraphs, or repeated reassurance.
`;
    }
...
- Use natural Telugu-in-English words only when they fit: nuvvu, nenu, em, enti, ela, enduku, avunu, ledu, ippudu, chala, baga, bagunnava, undi, unna, chestunna, chesanu, ra, raa, le, naku, neeku, meeru.
- Avoid words like: enna, iruka, pannra, poraa, adhu, illa, macha.
- Sound like a real person chatting, not like an assistant.
- If the user asks who you are or what your name is, reply: "I am Nandu."
- If the message is just a greeting, reply with a simple casual greeting like "Hi" or "Hey".
- Do not use filler lines, long paragraphs, repeated reassurance, or dramatic wording.
`;
}

function cleanReply(reply: string, messageBody: string): string {
    ...
    const isGreeting = /^(hi+|hii+|hello+|hey+|hai+|yo+|sup+|gm+|good morning|good night)\b/.test(messageBody.toLowerCase().trim());
    if (isGreeting) {
        cleaned = cleaned
            .split('\n')[0]
            .split(/(?<=[.!?])\s+/)[0]
...
export async function generateSmartReply(messageBody: string, senderName: string, chatHistory: { role: 'user' | 'assistant', content: string }[] = [], senderNumber: string = ""): Promise<string> {
    ...
    try {
        ...
        const rawReply = completion.choices[0]?.message?.content || "Sorry, I can't think of a reply (AI Error).";
        return cleanReply(rawReply, messageBody);
    } catch (error: any) {
